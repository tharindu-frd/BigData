...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://irisseta/scripts/ingest.sh']],
   #### Created a s3 bucket called scripts and inside that put ingest.sh file and transform.py files 
   ####  s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar   : this path is a constant but we may have to change the region 
   #### Purpose: The script-runner.jar is used to run custom scripts (such as shell scripts, Python scripts, or other executable files) as part of the Amazon EMR cluster's bootstrap actions, steps, or other operations.
   #### Location: This JAR file is stored in an S3 bucket managed by Amazon EMR. The location is specific to the us-east-1 region, but similar JAR files exist in other regions as well.
